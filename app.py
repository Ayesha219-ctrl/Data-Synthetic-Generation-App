    ...
    if uploaded_file is not None:
        try:
            ...
            if to_drop:
                ...
                df = df.drop(columns=to_drop)   # FIXED LINE
            else:
            ...
            if st.button("🚀 Train and Generate Synthetic Data"):
                try:
                    ...
                    if numeric_cols_final:
                        st.write("### Numeric Columns (Histogram + KDE + KS Test + Wasserstein Distance)")
                        SYNTH_COLORS = {
                            "CTGAN": "#FF7F0E",             # Orange
                            "GaussianCopula": "#2CA02C",    # Green
                        }
                        ...
                        for col in numeric_cols_final[:max_cols_plot]:
                            try:
                                ...
                                # ======================================================
                                #   SEPARATE HISTOGRAM PLOT (NO KDE HERE)
                                # ======================================================
                                ...
                                # Synthetic Histograms (COLOR ADDED)
                                for synth_name, synthetic_data in synthetic_datasets.items():
                                    ax_hist.hist(
                                        ...
                                        alpha=0.5,
                                        label=f"{synth_name}",
                                        color=SYNTH_COLORS.get(synth_name, "#9467BD")  # fallback color
                                    )

                                ax_hist.set_title(f"Histogram Comparison: {col}", color="#222222")
                                ax_hist.set_xlabel(col, color="#222222")
                                ax_hist.set_ylabel("Density", color="#222222")
                                ax_hist.tick_params(axis='both', colors='#222222')

                                leg = ax_hist.legend()
                                for t in leg.get_texts():
                                    t.set_color("#222222")

                                ax_hist.set_facecolor("#FFF3ED")
                                fig_hist.set_facecolor("#FFF3ED")

                                st.pyplot(fig_hist)
                                plt.close(fig_hist)

                                # ======================================================
                                #   SEPARATE KDE PLOT (WITH COLOR FILL)
                                # ======================================================
                                ...
                                # Synthetic KDEs (COLOR ADDED)
                                for synth_name, synthetic_data in synthetic_datasets.items():
                                    sns.kdeplot(
                                        synthetic_data[col].dropna(),
                                        ax=ax_kde,
                                        linewidth=2.0,
                                        label=f"{synth_name} KDE",
                                        fill=True,
                                        alpha=0.25,
                                        color=SYNTH_COLORS.get(synth_name, "#9467BD")
                                    )

                                ax_kde.set_title(f"KDE Comparison: {col}", color="#222222")
                                ax_kde.set_xlabel(col, color="#222222")
                                ax_kde.set_ylabel("Density", color="#222222")
                                ax_kde.tick_params(axis="both", colors="#222222")

                                leg = ax_kde.legend()
                                for t in leg.get_texts():
                                    t.set_color("#222222")

                                ax_kde.set_facecolor("#FFF6F4")
                                fig_kde.set_facecolor("#FFF6F4")

                                st.pyplot(fig_kde)
                                plt.close(fig_kde)

                                # ======================================================
                                #   STATISTICAL SCORES (UNCHANGED)
                                # ======================================================
                                for synth_name, synthetic_data in synthetic_datasets.items():
                                    synth_series = synthetic_data[col].dropna()

                                    ks_stat, ks_p, ks_status = perform_ks_test_fixed(
                                        real_series, synth_series, sample_size=ks_sample_size
                                    )

                                    norm_mean_diff, overlap, metric_status = calculate_similarity_metrics(
                                        real_series, synth_series
                                    )

                                    w_dist, w_status = calculate_wasserstein_distance(
                                        real_series, synth_series, sample_size=ks_sample_size
                                    )
...
# ================================
                    if len(numeric_cols_final) > 1:
                            ...
                            try:
                                ...
                                # === REAL DATA HEATMAP === #
                                sns.heatmap(
                                    df[numeric_cols_final].corr(),
                                    ax=axes[0],
                                    **heatmap_params
                                )
                                axes[0].set_title("Real Data Correlations", fontsize=12, color='#222222')
                                axes[0].tick_params(axis='both', labelsize=8, colors='#222222')
                                axes[0].set_facecolor('#FFEFEA')

                                # === SYNTHETIC HEATMAPS === #
                                for idx, (synth_name, synthetic_data) in enumerate(synthetic_datasets.items(), 1):
                                    sns.heatmap(
                                        synthetic_data[numeric_cols_final].corr(),
                                        ax=axes[idx],
                                        **heatmap_params
                                    )
                                    axes[idx].set_title(f"Synthetic ({synth_name})", fontsize=12, color='#222222')
                                    axes[idx].tick_params(axis='both', labelsize=8, colors='#222222')
                                    axes[idx].set_facecolor('#FFEFEA')

                                fig.set_facecolor('#FFEFEA')

                                plt.tight_layout()   # 🔥 Removes overlap
                                st.pyplot(fig)
                                plt.close(fig)
                    ...
                    for synth_name, summaries in summary_list.items():
                        if summaries:
                            ...
                            **Quality Assessment:**
                            ...
                            - ⚠️ **Acceptable**: KS < 0.2, P-value > 0.01, Mean Diff < 25%, Wasserstein Distance < 0.5
                            ...
                            # --------------------------
# Accuracy Assessment & Summary Report
# --------------------------

# Placeholder for correlation differences since correlations are not computed
                            corr_diff_info = {"Corr Diff": 0.0, "Status": "Not computed"}
                            ...
                            if overall_accuracy >= accuracy_threshold:
                                ...
                                st.error(f"⚠️ **FAILED**: Synthetic data is below the {accuracy_threshold}% accuracy threshold")
                    ...
                    for synth_name, summaries in summary_list.items():
                        if summaries:
                            ...
                            st.write(f"- {synth_name} Average Correlation Difference: 0.0000 (Not computed)")
                    ...
                    for synth_name, summaries in summary_list.items():
                        if summaries:
                            ...
                            overall_accuracy, accuracy_breakdown = calculate_overall_accuracy(
                                summaries, 
                                {"Corr Diff": 0.0, "Status": "Not computed"},
                                quality_report
                            )
                            ...
                            if overall_accuracy >= accuracy_threshold:
                                ...
                                st.warning(f"{synth_name}: ⚠️ Synthetic data fails accuracy threshold ({overall_accuracy:.1f}% < {accuracy_threshold}%). Consider parameter adjustments.")
